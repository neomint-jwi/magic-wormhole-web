...
const THEME_KEY: string = "wormhole-theme";
const TEXT_MAX_LENGTH: number = 10000;
const ENCRYPTION_MARKER: string = "WORMHOLE_ENCRYPTED_V1:";
...
async function decryptText(
  ...
  const encrypted: Uint8Array = Uint8Array.from(atob(base64), (c: string): number =>
    c.charCodeAt(0)
  );
...
async function encryptFile(file: File, password: string): Promise<File> {
...
async function decryptFile(
  ...
  return new File([decrypted.buffer as ArrayBuffer], metadata.name, { type: metadata.type });
}
